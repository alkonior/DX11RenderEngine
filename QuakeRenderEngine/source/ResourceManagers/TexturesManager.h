...
public:
    ...
    virtual void RegTexture(void* tx, int width, int height, size_t id) override;
    virtual void RegTexture(const TextureData& tx, size_t id) override;
    virtual void UpdateTexture(const TextureData& tx, size_t id) override;
    virtual void UpdateTexture(const ImageUpdate& updateData) override;
...
private:
    std::map<Renderer::string_id, TextureCache> colorTextures;
...
#pragma region Float3Textres
    ...
    bool WasIdUsed(size_t id) override;
