...
PresentationParameters GraphicsBase::GenParams(const RenderEngineInitStruct& Init) {
	PresentationParameters pp;

	pp.isFullScreen = false;
	pp.backBufferWidth = Init.width;
	pp.backBufferHeight = Init.height;
	pp.deviceWindowHandle1 = Init.hWnd1;
	pp.deviceWindowHandle2 = Init.hWnd2;
...
GraphicsBase::GraphicsBase(RenderEngineInitStruct init)
	:width(init.width), height(init.height), pRenderer(new D3D11Renderer(GenParams(init), 1)){
