...
Graphics::Graphics(RenderEngineInitStruct init)
	:GraphicsBase(init)
{
	ImGui::CreateContext();
...
void Graphics::DrawSetUserPolygon(MeshHashData model, UPModelMesh newModel, size_t textureId, UPDrawData data) {
	managerUP.DrawSet(model, newModel, texturesManger.GetImg(textureId), data);
}

MeshHashData Graphics::RegisterhUserPolygon(UPModelMesh model, bool dynamic) {
