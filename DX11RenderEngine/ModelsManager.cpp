...
void ModelsManager::RegisterFramedModel(const FramedModelData& model, size_t id) {
	...
	for (size_t i = 0; i < framesCount; i++) {
		...
		renderer->SetVertexBufferData(vertexBuffer->vertexBuffers[0], 0, (void*)model.frames[i].data(), (UINT)model.frames[i].size(), sizeof(float3), sizeof(float3), SetDataOptions::SETDATAOPTIONS_NONE);
	}
