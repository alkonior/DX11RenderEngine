...
void GBuffer::Resize(int width, int height)
{
	...
	device->CreateTexture2D(&texDesc, nullptr, &MetRougAoTex);
	...
	device->CreateShaderResourceView(MetRougAoTex, nullptr, &MetRougAoSRV);
	...
	device->CreateRenderTargetView(MetRougAoTex, nullptr, &MetRougAoRTV);
...
void GBuffer::Dispose()
{
	...
	SafeRelease(BloomTex);
	SafeRelease(BloomTex);
	SafeRelease(MetRougAoSRV);
	SafeRelease(MetRougAoRTV);
	SafeRelease(MetRougAoTex);
}
